...
// Restrict 'key' to only keys of the Project type
type SortConfigType = {
    key: keyof Project;
...
const ProjectTable = () => {
    ...
    const sortProjects = (key: keyof Project) => {
        ...
        const sortedProjects = [...projects].sort((a, b) => {
            ...
            if (a[key] > b[key]) return direction === "ascending" ? 1 : -1;
            return 0; // Handle equality properly
        });
                    ...
                    {currentProjects.map((project, index) => (
                        <tr key={index} className="border border-gray-700">
                            ...
                            <td className="px-4 py-2 w-[10rem]">
                                <span className={`${project.status === "Completed" ? "bg-green-500" : "bg-yellow-500"} p-1 rounded text-white`}>
                                    {project.status}
                                </span>
                            </td>
